...
//	private long conntime = recvTime - sendTime;
	...
	public HouseSocketConn(String ip, int port) {
		this.ip = ip;
		this.port = port;
	...
	public void conn() {
		...
		try {
			while (flag) {
				...
				LogUtils.w("创建家政服务器的socket-- ip = "+ ip+"/ port = " + port);
						...
						try {
							...
							if (dis.available() <= 0) {
								try {
									...
									body = DealHouseSocketData.getNetData();
									
									if(body != null){
										long recvTime = System.currentTimeMillis();
										LogUtils.d("house socket收到数据(缓存)--接收时间="+recvTime+"");
...
//							}
							...
							DealHouseSocketData.pushNetData(btContent);
							body = DealHouseSocketData.getNetData();
...
//								HandleNetHeadMsg headMsg = HandleNetHeadMsg.parseHeadMag(proBufHead);
								long recvTime = System.currentTimeMillis();
								LogUtils.d("house socket收到数据--接收时间="+recvTime+"");
								HandleMsgDistribute.getInstance().insertRecvMsg(recvTime, body);
								sendRecvService(recvTime);
...
//		}
	...
	public void sendRecvService(long recvTime) {
		...
		serviceIntent.setAction(Define.BROAD_CAST_RECV_DATA_HOUSE);
